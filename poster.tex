...
\begin{document}
\begin{frame}
  \begin{textblock}{0.245}(0.01, 0.05)
    ...
    \begin{block}{General approach}
      The core idea is the same as in Hammer (\emph{Bioinformatics}~(2011),~27~(13);~i137-i141)
      \begin{enumerate}
      \item Count and cluster homopolymer-space $k$-mers (we used $k = 16$)
      \item Fill consensus array for each read, using centers of clusters containing $k$-mers from the read
      \item Compute consensus for each homopolymer run in the read
      \end{enumerate}

      There are several substantial differences, however. Firstly, the basic unit of operation is homopolymer run, i.e.
      a pair $(\textrm{nucleotide,~length})$. Second, using Hamming distance in clustering is out of question due to
      abundance of indels in Ion Torrent data. Therefore, those tricks for fast clustering, that are based on Hamming
      distance properties, are, in the best case, only partially applicable for homopolymer-space $k$-mers.
  ...
  \begin{textblock}{0.35}(0.260, 0.05)
    \begin{block}{Results}
      ...
      \begin{figure}[h]
        ...
        \includegraphics[width=0.65\textwidth]{images/error_rate_vs_offset}
      \end{figure}
    \end{block}
  \end{textblock}

  \begin{textblock}{0.23}(0.260, 0.62)
    \begin{block}{Pairwise distance calculation}
      We use 5-base lookahead to compute distance between homopolymer-space $k$-mers. Helper
      table stores precomputed values for the functions
      \begin{eqnarray*}
        H_k: \left\{\mathrm{A, C, G, T}\right\}^{k} \times \left\{\mathrm{A, C, G,
          T}\right\}^{k} \to \left\{\mathrm{Insertion, Deletion,
          Mismatch}\right\}, \\
        k=1,2,3,4,5\,\mathrm{.}
      \end{eqnarray*}
    ...
    {\large Algorithm}
    \begin{algorithmic}
      \State \textbf{Input:} $x, y$ --- nucleotide sequences
      \State \textbf{Output:} $dist$ --- approximate Levenshtein distance between $x$ and $y$
      \vspace{0.3cm}
      \State $pos.x \gets 0$; $pos.y \gets 0$; $dist \gets 0$;
      \While{$pos.x < \mathrm{length}(x)\textrm{ and }pos.y < \mathrm{length}(y)$}
      \If{$x[pos.x] = y[pos.y]$}
      \State $pos.x \gets pos.x + 1$; $pos.y \gets pos.y + 1$;
      \Else
      \State $k \gets \mathrm{min}(5, \mathrm{length}(x) - pos.x, \mathrm{length}(y) - pos.y)$;
  ...
  \begin{textblock}{0.16}(0.495, 0.62)
  ...
  \begin{textblock}{0.08}(0.660, 0.62)
    ...
    \end{block}
  \end{textblock}
  \begin{textblock}{0.245}(0.750, 0.62)
